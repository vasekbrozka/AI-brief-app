 ...
 * chain of dots (the dot of the brief on screen fills with the day's
 * progress), tier title, and a one-shot celebration when every story is read.
 * The day itself counts after the first story (`started`); the ring keeps
 * filling until all are read (`done`). Colour escalates with the tier.
 */
export function WeekStreak({
  ...
  /** Share of the day's stories read, 0–1. */
  todayProgress: number;
  /** Every story read — the celebration. */
  done: boolean;
  /** At least one story read — the day counts. */
  started: boolean;
  ...
  const starter = currentStreak === 0 && !started;
  ...
  return (
    ...
    >
      ...
      <div className="sc__week" role="img" aria-label={`${currentStreak}`}>
        {week.map((d, i) => {
          // The brief on screen is the active day: its dot fills with progress
          // and completes with a check once every story is read.
          ...
          return (
            <div key={d.iso} className={`sc__day${chain ? ' has-chain' : ''}`}>
              <span
                ...
                style={active && !activeDone ? ({ ['--tp' as string]: todayProgress } as object) : undefined}
              >
                {activeDone && <Icon name="check" size={15} />}
                {d.done && !active && <Icon name="check" size={13} />}
              </span>
              <small className={active ? 'is-today' : undefined}>{wd(d.iso)}</small>
            </div>
          );
        })}
      </div>
